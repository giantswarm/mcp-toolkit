...
- Override `Option` functions on each of the three Init signals (defaults preserved, opt-in):
  - `tracing.WithPropagators` (replaces the default `TraceContext + Baggage` composite — use for B3 / Jaeger / custom interop), `tracing.WithSampler` (override the SDK default `ParentBased(AlwaysSample)` — e.g. ratio-based head sampling), `tracing.WithResourceOptions` (append extra `resource.Option` values for attributes / detectors).
  ...
  - `logging.WithExtraHandlers` (fan log records out to additional `slog.Handler` sinks — stderr mirror, file audit, secondary collector), `logging.WithBatchProcessorOptions` (tune queue size / export interval / export timeout in OTLP mode), `logging.WithResourceOptions`.
