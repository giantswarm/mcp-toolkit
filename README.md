...
| [`metrics`](./metrics) | OpenTelemetry meter-provider init driven by `OTEL_METRICS_EXPORTER` via autoexport — `otlp` (push to a collector), `prometheus` (self-hosted `/metrics:9464`), `console`, or `none`. Comma-separated values enable multiple exporters from one set of instruments. Pins `exemplar.TraceBasedFilter` so histogram observations recorded inside a sampled span attach the TraceID for Grafana's latency-bucket-to-trace pivot. |
